...
const C_PREGARA = [
 ...
 ['Il capo officina ti deve un debito','discount',0.25],['Il figlio del capo officina ti salda il debito','discount',0.5],['Il meccanico ti fa un favore','discount',0.5],
...
function priceFor(G,p,comp,lvl){ let price=pieceBasePrice(p,comp,lvl); if(lvl>p.comp[comp]+1) price*=2; if(p.discountNext) price=Math.round(price*(1-p.discountNext)); return price; }   // B: prezzo personale (sconto = % della carta giocata)
...
function actBuy(room,p,comp,lvl){
  ...
  p.money-=price; p.comp[comp]=lvl; if(p.lvlOwned&&!p.lvlOwned[comp].includes(lvl)) p.lvlOwned[comp].push(lvl); p.buysLeft--; p.discountNext=0;
...
function buildView(room, player){
  ...
  if(G.phase==='prep'){
    ...
    if(v.isYourTurn){
      ...
      v.track=trackView(G);
      v.me={ money:p.money, po:p.po, buysLeft:p.buysLeft, stats:statsOf(p), owned:ownedView(p), handCount:p.hand.length, prizeMult:(p.prizeMult||1), betMult:(p.betMult||1), quotaMod:(p.quotaMod||0), discount:!!p.discountNext, discountPct:p.discountNext||0,
